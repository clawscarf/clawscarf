...
/** Read the credential only in the browser; never include it in a GET request. */
const linkScript = `const code = new URLSearchParams(location.hash.slice(1)).get("code");
...
export const localSignInPagePolicy = `${signInPagePolicy}; script-src 'sha256-${createHash("sha256").update(linkScript).digest("base64")}'`;
